...
export default function KnowledgePage() {
 ...
 className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium bg-surface-100 border border-surface-200 hover:border-brand-300 dark:hover:border-brand-700 hover:text-brand-600 dark:hover:text-brand-400 transition-colors shrink-0">
 ...
 <div className="card mb-5 bg-surface-50 border border-surface-200">
 ...
 className={`badge cursor-pointer text-xs ${filterModule === m.id ?"text-white" :"badge-gray hover:bg-surface-200 dark:hover:bg-surface-600"}`}
 ...
 className={`badge cursor-pointer text-xs flex items-center gap-1 ${filterExam === e.id ?"text-white" :"badge-gray hover:bg-surface-200 dark:hover:bg-surface-600"}`}
...
function TopicNode({ topic, children, allTopics, expanded, onToggleExpand, onToggleStatus, onEdit, onDelete, onAddChild, onSetLevel, exams, depth }: {
 ...
 {hasChildren ? (isExpanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />) : <span className="w-2 h-2 rounded-full bg-surface-200" />}
        ...
        <span className="flex-1 text-sm text-surface-800 font-medium truncate">{topic.title}</span>
 ...
 <button onClick={() => onAddChild(topic.id)} className="p-1.5 rounded-lg hover:bg-surface-200 dark:hover:bg-surface-700 text-surface-400 hover:text-brand-500 dark:hover:text-brand-400" title={t("knowledge.subTopic")}>
            <Plus size={13} />
          </button>
 <button onClick={() => onEdit(topic)} className="p-1.5 rounded-lg hover:bg-surface-200 dark:hover:bg-surface-700 text-surface-400" title={t("documents.edit")}>
            <Pencil size={13} />
          </button>
 <button onClick={() => onDelete(topic.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900 text-surface-400 hover:text-red-500 dark:hover:text-red-400" title={t("documents.delete")}>
            <Trash2 size={13} />
          </button>
        </div>
      </div>
...
function SRReviewModal({ topics, contextLabel, onClose }: { topics: Topic[]; contextLabel?: string; onClose: () => void }) {
 ...
 <div className="bg-surface-50 rounded-2xl shadow-xl w-full max-w-md text-center p-4 sm:p-8">
 ...
 <p className="text-xs text-surface-500">{t("knowledge.statusRetry")}</p>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
 <div className="bg-surface-50 rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
 ...
 <p className="text-surface-600 mt-3 text-sm bg-surface-100 rounded-xl p-4">{current.description}</p>
          )}
        </div>
