...
export default function LeaderboardPage() {
 ...
 <div className="grid grid-cols-12 gap-3 px-3 sm:px-4 py-3 bg-surface-50 font-semibold text-xs text-surface-600 uppercase">
              ...
              {rest.map((entry, idx) => {
                ...
                return (
                  <div
                    key={entry.id}
                    className={`grid grid-cols-12 gap-3 px-3 sm:px-4 py-3 items-center hover:bg-surface-50 dark:hover:bg-surface-700/50 transition-colors ${
                      isCurrentUser ? "bg-brand-50 dark:bg-brand-950/30 border-l-4 border-brand-500 dark:border-brand-400" : ""
                    }`}
                  >
                    {/* Rank */}
 ...
 <div className="text-xs text-surface-500 truncate">@{entry.username}</div>
                    ...
                    {/* XP */}
                    <div className="col-span-2 text-right font-semibold text-brand-600 dark:text-brand-400 flex items-center justify-end gap-1">
                      <Zap size={12} />
                      {entry.xp_total.toLocaleString()}
                    </div>
                  </div>
 ...
 <div className="text-xs text-surface-500 mb-2 font-semibold uppercase">{t("leaderboard.you")}</div>
            <div className="card bg-brand-50 dark:bg-brand-950/30 border-brand-200 dark:border-brand-800">
              <div className="grid grid-cols-12 gap-3 items-center px-3 sm:px-4 py-3">
                {/* Rank */}
                <div className="col-span-1 font-bold text-brand-700 dark:text-brand-400">{data.userRank}</div>
                ...
                <div className="col-span-7 flex items-center gap-2">
                  {data.currentUser.avatar_url ? (
                    ...
                    <div className="w-8 h-8 rounded-full bg-brand-200 dark:bg-brand-900/40 flex items-center justify-center text-xs font-bold text-brand-700 dark:text-brand-300">
