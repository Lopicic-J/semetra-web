...
export default function DeveloperPage() {
 ...
 <div className="overflow-x-auto flex items-center gap-2 bg-[rgb(var(--card-bg))] rounded-lg px-3 py-2 border border-green-200 dark:border-green-800 font-mono text-xs break-all">
 ...
 <button onClick={fetchKeys} className="p-2.5 rounded-lg hover:bg-surface-100 dark:hover:bg-surface-700 transition-colors text-surface-500"><RefreshCw size={16} /></button>
 ...
 <input type="text" value={newKeyName} onChange={(e) => setNewKeyName(e.target.value)} placeholder="Key-Name (z.B.'Production App')" className="w-full px-3 py-2 rounded-lg border border-surface-200 bg-[rgb(var(--card-bg))] text-surface-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 dark:focus:ring-brand-400" />
              <div>
 ...
 <input type="checkbox" checked={newKeyScopes.includes(scope.value)} onChange={(e) => setNewKeyScopes(e.target.checked ? [...newKeyScopes, scope.value] : newKeyScopes.filter((s) => s !== scope.value))} className="rounded border-surface-300 text-brand-600 dark:text-brand-500 focus:ring-brand-500 dark:focus:ring-brand-400" />
                      {scope.label}
                    </label>
                  ))}
                </div>
 ...
 <label className="text-xs text-surface-600 whitespace-nowrap">Ablauf in</label>
 <select value={newKeyExpiry} onChange={(e) => setNewKeyExpiry(Number(e.target.value))} className="text-sm px-3 py-2 rounded-lg border border-surface-200 bg-[rgb(var(--card-bg))] text-surface-900 dark:text-white">
 ...
 <button onClick={() => setShowNewKey(false)} className="px-4 py-2 text-sm text-surface-600 hover:bg-surface-100 dark:hover:bg-surface-700 rounded-lg transition-colors">Abbrechen</button>
 ...
 <p className="text-xs text-surface-500 font-mono break-all">{k.key_prefix}...</p>
                  </div>
 ...
 <button onClick={() => handleDeleteKey(k.id)} className="p-2 rounded-lg text-surface-300 hover:text-danger-600 dark:hover:text-danger-400 hover:bg-danger-50 dark:hover:bg-danger-900/20 transition-colors opacity-0 group-hover:opacity-100"><Trash2 size={14} /></button>
                </Card>
              ))}
            </div>
          )}
 ...
 <p className="text-sm text-surface-600">{webhooks.length} Webhook{webhooks.length !== 1 ?"s" :""}</p>
            <div className="flex gap-2">
 <button onClick={fetchWebhooks} className="p-2.5 rounded-lg hover:bg-surface-100 dark:hover:bg-surface-700 transition-colors text-surface-500"><RefreshCw size={16} /></button>
          ...
          {showNewWh && (
            <Card className="p-4 space-y-3 border-brand-200 dark:border-brand-800 bg-brand-50/30 dark:bg-brand-900/10">
 <input type="url" value={newWhUrl} onChange={(e) => setNewWhUrl(e.target.value)} placeholder="https://example.com/webhook" className="w-full px-3 py-2 rounded-lg border border-surface-200 bg-[rgb(var(--card-bg))] text-surface-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 dark:focus:ring-brand-400 break-all" />
              <div>
 ...
 <input type="checkbox" checked={newWhEvents.includes(ev.value)} onChange={(e) => setNewWhEvents(e.target.checked ? [...newWhEvents, ev.value] : newWhEvents.filter((v) => v !== ev.value))} className="rounded border-surface-300 text-brand-600 dark:text-brand-500 focus:ring-brand-500 dark:focus:ring-brand-400" />
                      {ev.label}
                    </label>
                  ))}
                </div>
              ...
              <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 pt-1">
 <button onClick={() => setShowNewWh(false)} className="px-4 py-2 text-sm text-surface-600 hover:bg-surface-100 dark:hover:bg-surface-700 rounded-lg transition-colors">Abbrechen</button>
 ...
 <button onClick={() => handleDeleteWebhook(wh.id)} className="p-2 rounded-lg text-surface-300 hover:text-danger-600 dark:hover:text-danger-400 hover:bg-danger-50 dark:hover:bg-danger-900/20 transition-colors opacity-0 group-hover:opacity-100"><Trash2 size={14} /></button>
                </Card>
              ))}
            </div>
          )}
 ...
 <button onClick={fetchUsage} className="p-2.5 rounded-lg hover:bg-surface-100 dark:hover:bg-surface-700 transition-colors text-surface-500"><RefreshCw size={16} /></button>
          </div>
 ...
 <div className="absolute -top-8 left-1/2 -translate-x-1/2 hidden group-hover/bar:block bg-surface-900 text-white dark:text-white text-[10px] px-2 py-1 rounded whitespace-nowrap z-10">
 ...
 <span className="text-xs text-surface-400 sm:w-10 sm:text-right">{pct.toFixed(0)}%</span>
                        </div>
                      );
                    })}
                  </div>
