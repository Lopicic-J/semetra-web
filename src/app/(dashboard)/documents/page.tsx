...
export default function DocumentsPage() {
 ...
 className={`bg-surface-100 border rounded-xl p-2 sm:p-3 text-center transition ${viewMode ==="flow" ?"border-brand-500 dark:border-brand-400" :"border-surface-200 hover:border-surface-300 dark:hover:border-surface-600"}`}
        >
 ...
 filterModule === m.id ?"ring-2 ring-white/30" :"hover:brightness-110 dark:hover:brightness-125"
                }`}
 ...
 <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-surface-400 flex-shrink-0" />
          <input
            value={searchQ}
            onChange={e => setSearchQ(e.target.value)}
            placeholder={t("documents.search")}
 className="w-full bg-surface-100 border border-surface-200 rounded-lg pl-10 pr-3 py-1.5 sm:py-2 text-xs sm:text-sm text-surface-900 dark:text-white placeholder:text-surface-400 dark:placeholder:text-surface-500 focus:border-brand-500 dark:focus:border-brand-400 focus:outline-none transition"
          />
        </div>
        <select
          value={filterModule}
          onChange={e => setFilterModule(e.target.value)}
 className="bg-surface-100 border border-surface-200 rounded-lg px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm text-surface-900 dark:text-white"
        >
 ...
 className="px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg bg-surface-100 text-surface-500 hover:text-surface-900 dark:hover:text-surface-100 text-xs transition flex-shrink-0"
          >
            {t("grades.filterAll")}
          </button>
        )}
...
function DocCard({ doc, modules, onEdit, onDelete, onTogglePin }: {
  ...
  return (
    <div
 className="bg-surface-100 border border-surface-200 rounded-xl p-2 sm:p-4 hover:border-surface-300 dark:hover:border-surface-600 transition group"
 ...
 <a href={doc.url} target="_blank" rel="noopener" className="p-1 sm:p-1.5 text-surface-500 hover:text-blue-400 dark:hover:text-blue-300 transition" title={t("documents.open")}><ExternalLink size={13} /></a>
 <button onClick={onTogglePin} className={`p-1 sm:p-1.5 transition ${doc.pinned ?"text-amber-400 dark:text-amber-300" :"text-surface-500 hover:text-amber-400 dark:hover:text-amber-300"}`} title={doc.pinned ? t("documents.detach") : t("documents.pin")}><Pin size={13} /></button>
 <button onClick={onEdit} className="p-1 sm:p-1.5 text-surface-500 hover:text-surface-900 dark:hover:text-surface-100 transition" title={t("documents.edit")}><Pencil size={13} /></button>
 <button onClick={onDelete} className="p-1 sm:p-1.5 text-surface-500 hover:text-red-400 dark:hover:text-red-300 transition" title={t("documents.delete")}><Trash2 size={13} /></button>
        </div>
      </div>
    </div>
  );
...
function DocListRow({ doc, modules, onEdit, onDelete }: {
  ...
  return (
 <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-surface-100 border border-surface-200 rounded-lg hover:border-surface-300 dark:hover:border-surface-600 transition group">
      <span style={{ color: cfg.color }}>{cfg.icon}</span>
...
function DocFlowView({ items, onOpenDoc }: { items: DocFlowItem[]; onOpenDoc: (d: Doc) => void }) {
 ...
 <a href={item.url} target="_blank" rel="noopener" className="p-1 text-surface-400 hover:text-blue-400 dark:hover:text-blue-300 transition" onClick={e => e.stopPropagation()} title={t("documents.open")}>
...
function PdfToolsModal({ onClose }: { onClose: () => void }) {
 ...
 activeTab ==="merge" ?"bg-surface-100 shadow text-brand-700 dark:text-brand-400" :"text-surface-500 hover:text-surface-700 dark:hover:text-surface-200"
                }`}>
              ...
              <button onClick={() => setActiveTab("split")}
                className={`px-2 sm:px-3 py-1.5 rounded-md text-xs font-medium transition whitespace-nowrap ${
 activeTab ==="split" ?"bg-surface-100 shadow text-brand-700 dark:text-brand-400" :"text-surface-500 hover:text-surface-700 dark:hover:text-surface-200"
                }`}>
 ...
 <button onClick={onClose} className="p-2 rounded-lg hover:bg-surface-100 dark:hover:bg-surface-700 text-surface-400 hover:text-surface-600 dark:hover:text-surface-300 transition flex-shrink-0">
            <X size={18} />
          </button>
        </div>
        {/* Content */}
...
function PdfMergePanel() {
 ...
 className="w-full mb-4 px-4 py-5 rounded-xl border-2 border-dashed border-surface-300 text-surface-500 text-sm hover:border-brand-400 dark:hover:border-brand-400 hover:text-brand-600 dark:hover:text-brand-400 transition flex flex-col items-center gap-1.5 active:scale-95 touch-friendly">
        <Upload size={18} />
 ...
 dragOverIdx === i ?"border-brand-400 dark:border-brand-400 ring-2 ring-brand-200 dark:ring-brand-900" : dragIdx === i ?"opacity-40 border-surface-300" :"border-surface-200 hover:border-surface-300 dark:hover:border-surface-600"
                }`}
              >
                {/* Order badge */}
...
function PdfSplitPanel() {
 ...
 <p className="text-[11px] text-surface-400 mt-0.5">{t("documents.pdfClickToAssign")}</p>
      ...
      {!file ? (
        <button onClick={() => fileInputRef.current?.click()}
 className="w-full px-4 py-5 rounded-xl border-2 border-dashed border-surface-300 text-surface-500 text-sm hover:border-brand-400 dark:hover:border-brand-400 hover:text-brand-600 dark:hover:text-brand-400 transition flex flex-col items-center gap-1.5 active:scale-95 touch-friendly">
          <Upload size={18} />
 ...
 <div className="flex items-center justify-center py-6 text-surface-400 text-sm gap-2">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 mb-3 px-1">
            <div className="flex items-center gap-2 sm:gap-3">
 <span className="text-xs font-medium text-surface-700 whitespace-nowrap">{t("documents.pdfNumOutputs")}:</span>
              <div className="flex items-center gap-1">
                <button onClick={() => changeNumOutputs(-1)} disabled={numOutputs <= 2}
 className="w-6 h-6 rounded-md bg-surface-100 hover:bg-surface-200 dark:hover:bg-surface-600 text-surface-600 flex items-center justify-center disabled:opacity-30 transition">
                  <Minus size={12} />
                </button>
 ...
 className="text-xs text-surface-400 hover:text-red-500 dark:hover:text-red-400 transition">
              {t("documents.pdfSelectFile")}
            </button>
          </div>
...
function DocModal({
 ...
 <button onClick={onClose} className="text-surface-500 hover:text-surface-900 dark:hover:text-surface-100 transition flex-shrink-0"><X size={20} /></button>
        </div>
 ...
 mode ==="link" ?"bg-surface-100 text-surface-900 dark:text-white shadow-sm" :"text-surface-500 hover:text-surface-700 dark:hover:text-surface-300"
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-md text-xs sm:text-sm font-medium transition ${
 mode ==="upload" ?"bg-surface-100 text-surface-900 dark:text-white shadow-sm" :"text-surface-500 hover:text-surface-700 dark:hover:text-surface-300"
              }`}
            >
 ...
 <label className="block text-xs sm:text-sm font-medium text-surface-800 mb-1.5">{t("documents.typeLink")}</label>
            <input
              ...
              placeholder="https://..."
 className="w-full bg-surface-50 border border-surface-200 rounded-lg px-3 py-1.5 sm:py-2.5 text-xs sm:text-sm text-surface-900 dark:text-white placeholder:text-surface-400 dark:placeholder:text-surface-500 mb-3 focus:border-brand-500 dark:focus:border-brand-400 focus:outline-none transition"
              autoFocus
            />
          </>
        )}
 ...
 className="w-full border-2 border-dashed border-surface-300 hover:border-brand-400 dark:hover:border-brand-400 rounded-xl p-3 sm:p-6 mb-3 flex flex-col items-center gap-2 transition-colors group active:scale-95 touch-friendly"
              >
 <Upload size={28} className="text-surface-300 group-hover:text-brand-500 dark:group-hover:text-brand-400 transition-colors" />
 <span className="text-sm font-medium text-surface-500 group-hover:text-surface-700 dark:group-hover:text-surface-300">{t("documents.typeFile")}</span>
 ...
 className="p-1 rounded hover:bg-surface-200 dark:hover:bg-surface-600 text-surface-400 hover:text-surface-600 dark:hover:text-surface-300 shrink-0 transition">
                  <X size={14} />
                </button>
              </div>
            )}
 ...
 className="w-full bg-surface-50 border border-surface-200 rounded-lg px-3 py-1.5 sm:py-2 text-xs sm:text-sm text-surface-900 dark:text-white mb-2"
        >
 ...
 className="flex-1 bg-surface-50 border border-surface-200 rounded-lg px-3 py-1.5 sm:py-2 text-xs sm:text-sm text-surface-900 dark:text-white"
            >
 ...
 className="w-full bg-surface-50 border border-surface-200 rounded-lg px-3 py-1.5 sm:py-2 text-xs sm:text-sm text-surface-900 dark:text-white placeholder:text-surface-400 dark:placeholder:text-surface-500 mb-3 sm:mb-4 focus:border-brand-500 dark:focus:border-brand-400 focus:outline-none transition"
        />
