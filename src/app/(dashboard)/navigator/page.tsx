  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 p-3 sm:p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-surface-900 flex items-center gap-2.5">
      ...
      <div className="relative group">
        ...
        {search ? (
          ...
          <kbd className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] text-surface-300 border border-surface-200 rounded px-1.5 py-0.5 font-mono hidden sm:inline">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  {stat.urgent && <AlertTriangle size={12} className="text-red-500" />}
                </div>
      ...
      {!search && !activeGroup && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {dueFlashcards > 0 && (
            <Link href="/flashcards" className="flex items-center gap-3 bg-gradient-to-r from-violet-50 to-purple-50 border border-violet-200 rounded-xl p-4 hover:shadow-md transition-all group">
              <div className="w-10 h-10 rounded-xl bg-violet-100 flex items-center justify-center shrink-0">
                <Layers size={18} className="text-violet-600" />
              </div>
              <div className="min-w-0">
                <p className="text-sm font-semibold text-violet-900 group-hover:text-violet-700">{dueFlashcards} {t("navigator.cardsDueAction") || "Karten wiederholen"}</p>
                <p className="text-[11px] text-violet-500">{t("navigator.cardsDueHint") || "Spaced Repetition — jetzt fällig"}</p>
              </div>
              <ArrowRight size={16} className="text-violet-300 group-hover:text-violet-500 transition ml-auto shrink-0" />
            </Link>
          )}
          {dueTasks > 0 && (
            <Link href="/tasks" className="flex items-center gap-3 bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200 rounded-xl p-4 hover:shadow-md transition-all group">
              <div className="w-10 h-10 rounded-xl bg-emerald-100 flex items-center justify-center shrink-0">
                <CheckSquare size={18} className="text-emerald-600" />
              </div>
              <div className="min-w-0">
                <p className="text-sm font-semibold text-emerald-900 group-hover:text-emerald-700">{dueTasks} {t("navigator.tasksOpenAction") || "Tasks offen"}</p>
                <p className="text-[11px] text-emerald-500">{t("navigator.tasksOpenHint") || "Aufgaben anschauen und abhaken"}</p>
              </div>
              <ArrowRight size={16} className="text-emerald-300 group-hover:text-emerald-500 transition ml-auto shrink-0" />
            </Link>
          )}
          <Link href="/timer" className="flex items-center gap-3 bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-xl p-4 hover:shadow-md transition-all group">
            <div className="w-10 h-10 rounded-xl bg-amber-100 flex items-center justify-center shrink-0">
              <Timer size={18} className="text-amber-600" />
            </div>
            <div className="min-w-0">
              <p className="text-sm font-semibold text-amber-900 group-hover:text-amber-700">{t("navigator.startLearning") || "Lernzeit starten"}</p>
              <p className="text-[11px] text-amber-500">{t("navigator.startLearningHint") || "Timer starten und fokussiert lernen"}</p>
            </div>
            <ArrowRight size={16} className="text-amber-300 group-hover:text-amber-500 transition ml-auto shrink-0" />
          </Link>
        </div>
      )}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-semibold text-surface-900 group-hover:text-brand-600 transition-colors">{f.label}</span>
                          {f.pro && (
                            <span className="text-[9px] bg-brand-900 text-brand-300 px-1.5 py-0.5 rounded font-bold">PRO</span>
                          )}
                        </div>
                        <p className="text-[11px] text-surface-500 mt-0.5 leading-relaxed line-clamp-2">{f.description}</p>
                      </div>
                      <ArrowRight size={14} className="text-surface-200 group-hover:text-brand-400 transition shrink-0 mt-1" />
                    </div>
                  </Link>
                );
              })}
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16">
          ...
          <button
            ...
            className="mt-3 text-brand-600 text-sm hover:text-brand-700 font-medium"
          >
