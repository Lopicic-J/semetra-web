...
function UserRow({
 ...
 className="px-3 py-1.5 text-xs font-medium bg-surface-100 text-surface-700 rounded-lg hover:bg-surface-200 dark:hover:bg-surface-600 transition"
          ...
          {/* ─── Studiengang editing (students only) ─── */}
          {(user.user_role === "student") && user.institution_id && (
            <StudiengangEditor userId={user.id} institutionId={user.institution_id} onRefresh={onRefresh} />
          )}
...
function StudiengangEditor({ userId, institutionId, onRefresh }: { userId: string; institutionId: string; onRefresh: () => void }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`/api/academic/programs?institution_id=${institutionId}`);
        if (!res.ok) throw new Error();
        const data = await res.json();
  ...
  async function handleSave() {
    ...
    try {
      ...
      const res = await fetch("/api/admin/users", {
        ...
        body: JSON.stringify({
          user_id: userId,
          active_program_id: selectedProgramId,
          study_program: program?.name || null,
        }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.error || "Studiengang konnte nicht aktualisiert werden");
      }
      toast.success("Studiengang aktualisiert");
      setEditing(false);
      onRefresh();
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Fehler");
  ...
  if (loading) {
    return (
      <div>
        <p className="text-xs font-semibold text-surface-500 uppercase tracking-wider mb-2">Studiengang</p>
  ...
  if (programs.length === 0) {
    return (
      <div>
        <p className="text-xs font-semibold text-surface-500 uppercase tracking-wider mb-2">Studiengang</p>
        <p className="text-sm text-surface-500">Keine Studiengänge für diese Institution hinterlegt</p>
      </div>
    );
  }
      ...
      {editing ? (
        <div className="flex items-center gap-2">
          <select
            ...
            className="flex-1 px-3 py-2 border border-surface-300 rounded-lg bg-[rgb(var(--card-bg))] text-surface-900 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
          >
          ...
          <button
            ...
            className="px-3 py-2 bg-brand-600 text-white rounded-lg hover:bg-brand-700 text-xs font-medium disabled:opacity-50"
          >
            {saving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : "Speichern"}
          </button>
          <button onClick={() => setEditing(false)} className="p-2 text-surface-600 hover:text-surface-900">
            <X className="w-4 h-4" />
          </button>
        </div>
      ) : (
