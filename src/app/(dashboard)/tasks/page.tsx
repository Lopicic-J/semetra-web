...
export default function TasksPage() {
 ...
 <p className="text-surface-500 text-sm mt-0.5">{t("tasks.subtitle", { open: tasks.filter(tk => tk.status !=="done").length, done: tasks.filter(tk => tk.status ==="done").length })}</p>
      ...
      {/* Filter tabs */}
 <div className="flex gap-1 p-1 bg-surface-100 rounded-xl mb-5 w-full overflow-x-auto">
 ...
 className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${filter === s ?"bg-surface-50 text-surface-900 dark:text-white shadow-sm" :"text-surface-500 hover:text-surface-700 dark:hover:text-surface-300"}`}>
              {label}
            </button>
          );
        })}
...
function TaskRow({ task, modules, onToggle, onEdit, onDelete, isExpanded, onToggleExpand }: {
  ...
  return (
 <div className={`flex flex-col sm:flex-row sm:items-center gap-3 p-3 rounded-xl border transition-colors group cursor-pointer ${task.status ==="done" ?"bg-surface-50 border-surface-100" :"bg-surface-100 border-surface-100 hover:border-brand-200 dark:hover:border-brand-700"} ${isExpanded ?"rounded-b-none border-b-0" :""}`}
      onClick={onToggleExpand}>
      <button onClick={(e) => { e.stopPropagation(); onToggle(task); }}
 className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${task.status ==="done" ?"bg-green-500 border-green-500 text-white" :"border-surface-300 hover:border-brand-400 dark:hover:border-brand-600"}`}>
        {task.status === "done" && <Check size={11} />}
      </button>

      <div className="flex-1 min-w-0">
 <p className={`text-sm font-medium ${task.status ==="done" ?"line-through text-surface-400" :"text-surface-800 dark:text-white"}`}>{task.title}</p>
        <div className="flex flex-wrap gap-2 mt-0.5">
          {task.due_date && (
 <span className={`text-xs ${isOverdue ?"text-red-500 dark:text-red-400 font-medium" :"text-surface-400"}`}>
 ...
 <button onClick={(e) => { e.stopPropagation(); onEdit(task); }} className="p-1.5 rounded-lg hover:bg-surface-200 dark:hover:bg-surface-700 text-surface-400"><Pencil size={13} /></button>
 <button onClick={(e) => { e.stopPropagation(); onDelete(task.id); }} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900 text-surface-400 hover:text-red-500 dark:hover:text-red-400"><Trash2 size={13} /></button>
        </div>
...
function TaskDetailPanel({ task, modules }: {
  ...
  return (
 <div className="bg-surface-50 border border-t-0 border-surface-100 rounded-b-xl p-3 sm:p-4 space-y-3">
      {/* Description */}
      {task.description && (
        <div>
 ...
 📅 {t("tasks.detailDue")}: <span className={`font-medium ${new Date(task.due_date) < new Date() && task.status !=="done" ?"text-red-600 dark:text-red-400" :"text-surface-700"}`}>{formatDate(task.due_date)}</span>
          </span>
        )}
        <span>
 ...
 className="flex items-center gap-1 px-2.5 py-1 text-xs font-medium rounded-lg bg-surface-100 border border-surface-200 text-surface-600 hover:border-brand-300 dark:hover:border-brand-700 hover:text-brand-600 dark:hover:text-brand-400 transition-colors">
 ...
 className="p-1.5 rounded-lg hover:bg-surface-200 dark:hover:bg-surface-700 text-surface-400"><X size={14} /></button>
          </form>
        )}
 ...
 <p className="text-xs text-surface-400 text-center py-3">{t("tasks.noAttachments")}</p>
          ...
          <div className="space-y-1.5">
            {attachments.map(att => (
 <div key={att.id} className="flex items-center gap-2.5 p-2 rounded-lg bg-surface-100 border border-surface-100 group/att hover:border-brand-200 dark:hover:border-brand-700 transition-colors">
 ...
 className="p-1 rounded hover:bg-surface-200 dark:hover:bg-surface-800 text-surface-400 hover:text-brand-600 dark:hover:text-brand-400 transition-colors"
                  title={t("tasks.openLink")}>
                  <ExternalLink size={13} />
                </a>
                <button onClick={() => remove(att)}
 className="p-1 rounded hover:bg-red-50 dark:hover:bg-red-900 text-surface-300 hover:text-red-500 dark:hover:text-red-400 opacity-0 group-hover/att:opacity-100 transition-all"
...
function TaskModal({ initial, modules, onClose, onSaved }: {
 ...
 <div className="bg-surface-50 rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
